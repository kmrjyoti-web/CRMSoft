...
var TestGroupRunnerService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TestGroupRunnerService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../../../../core/prisma/prisma.service");
let TestGroupRunnerService = TestGroupRunnerService_1 = class TestGroupRunnerService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(TestGroupRunnerService_1.name);
    }
    async execute(executionId, group, authToken) {
        ...
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            if (step.dependsOn?.some((depId) => skippedStepIds.has(depId))) {
                const skipped = {
                    stepId: step.id,
                    stepName: step.name,
                    status: 'SKIPPED',
            ...
            if (stepResult.status === 'PASS') {
                passed++;
                if (stepResult.savedValues)
                    Object.assign(savedValues, stepResult.savedValues);
            }
    ...
    async executeStep(step, savedValues, authToken) {
        ...
        try {
            ...
            const [method, ...pathParts] = resolvedEndpoint.split(' ');
            ...
            if (step.saveAs && responseBody) {
                for (const [key, pathExpr] of Object.entries(step.saveAs)) {
            ...
            return {
                ...
                errorMessage: (error instanceof Error ? error.message : String(error))?.substring(0, 1000),
            };
        }
    }
    resolveTemplateVars(obj, values) {
        if (obj === null || obj === undefined)
            return obj;
        const str = JSON.stringify(obj);
        const resolved = str.replace(/\{\{(\w+)\}\}/g, (_, key) => key in values ? String(values[key]) : `{{${key}}}`);
        try {
    ...
    evaluateAssertion(actual, operator, expected) {
        switch (operator) {
...
exports.TestGroupRunnerService = TestGroupRunnerService;
exports.TestGroupRunnerService = TestGroupRunnerService = TestGroupRunnerService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], TestGroupRunnerService);
//# sourceMappingURL=test-group-runner.service.js.map