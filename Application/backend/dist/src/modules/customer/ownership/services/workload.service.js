...
var WorkloadService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkloadService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../../../core/prisma/prisma.service");
let WorkloadService = WorkloadService_1 = class WorkloadService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(WorkloadService_1.name);
    }
    async getDashboard(params) {
        const where = { status: 'ACTIVE', userType: 'EMPLOYEE' };
        if (params.roleId)
    ...
    async getRebalanceSuggestions() {
        ...
        for (const from of overloaded) {
            ...
            const entities = await this.prisma.working.entityOwner.findMany({
                where: { userId: from.userId, entityType: entityType, isActive: true },
    ...
    async recalculateCounts(userId) {
        const counts = await this.getActualCounts(userId);
        const total = counts.leads + counts.contacts + counts.organizations + counts.quotations;
...
exports.WorkloadService = WorkloadService = WorkloadService_1 = __decorate([
