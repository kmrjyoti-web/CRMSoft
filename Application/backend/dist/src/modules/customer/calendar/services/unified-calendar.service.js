...
exports.UnifiedCalendarService = void 0;
...
let UnifiedCalendarService = class UnifiedCalendarService {
    constructor(prisma, taskAssignment) {
        this.prisma = prisma;
        this.taskAssignment = taskAssignment;
    }
    async getUnifiedView(userId, tenantId, roleLevel, startDate, endDate, sources, search) {
        ...
        const fetchers = [];
        if (allSources.includes('TASK'))
            fetchers.push(this.fetchTasks(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('ACTIVITY'))
            fetchers.push(this.fetchActivities(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('DEMO'))
            fetchers.push(this.fetchDemos(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('TOUR_PLAN'))
            fetchers.push(this.fetchTourPlans(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('REMINDER'))
            fetchers.push(this.fetchReminders(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('FOLLOW_UP'))
            fetchers.push(this.fetchFollowUps(tenantId, visibleUserIds, startDate, endDate));
        if (allSources.includes('SCHEDULED_EVENT'))
            fetchers.push(this.fetchScheduledEvents(tenantId, userId, roleLevel, visibleUserIds, startDate, endDate));
    ...
    async getTeamView(userId, tenantId, roleLevel, startDate, endDate, teamUserIds) {
    ...
    async getStats(userId, tenantId, roleLevel) {
        const now = new Date();
    ...
    async getVisibleUserIds(userId, roleLevel, tenantId) {
        if (roleLevel <= 1)
            return [];
        if (roleLevel <= 3) {
    ...
    applyUserFilter(tenantId, userIds, userField) {
        const where = { tenantId: { in: [tenantId, ''] } };
        if (userIds.length > 0)
            where[userField] = { in: userIds };
        return where;
    }
    async fetchTasks(tenantId, userIds, start, end) {
    ...
    async fetchActivities(tenantId, userIds, start, end) {
    ...
    async fetchDemos(tenantId, userIds, start, end) {
        const where = { ...this.applyUserFilter(tenantId, userIds, 'conductedById'), scheduledAt: { gte: start, lte: end } };
    ...
    async fetchTourPlans(tenantId, userIds, start, end) {
        const where = { ...this.applyUserFilter(tenantId, userIds, 'salesPersonId'), planDate: { gte: start, lte: end } };
    ...
    async fetchReminders(tenantId, userIds, start, end) {
        const where = { ...this.applyUserFilter(tenantId, userIds, 'recipientId'), isActive: true, scheduledAt: { gte: start, lte: end } };
    ...
    async fetchFollowUps(tenantId, userIds, start, end) {
        const where = { ...this.applyUserFilter(tenantId, userIds, 'assignedToId'), isActive: true, dueDate: { gte: start, lte: end } };
    ...
    async fetchScheduledEvents(tenantId, userId, roleLevel, userIds, start, end) {
        const where = { tenantId, isActive: true, startTime: { gte: start, lte: end } };
...
exports.UnifiedCalendarService = UnifiedCalendarService;
exports.UnifiedCalendarService = UnifiedCalendarService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        task_assignment_service_1.TaskAssignmentService])
], UnifiedCalendarService);
//# sourceMappingURL=unified-calendar.service.js.map