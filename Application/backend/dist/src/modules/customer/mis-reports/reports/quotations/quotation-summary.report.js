...
exports.QuotationSummaryReport = void 0;
...
let QuotationSummaryReport = class QuotationSummaryReport {
    constructor(prisma, drillDownSvc, resolver) {
        this.prisma = prisma;
        this.drillDownSvc = drillDownSvc;
        this.resolver = resolver;
        this.code = 'QUOTATION_SUMMARY';
        this.name = 'Quotation Summary';
        this.category = 'QUOTATION';
        this.description = 'Overall quotation KPIs including volume, value, acceptance rates, and response times';
    ...
    async generate(params) {
        ...
        const bySt = (st) => quotations.filter(q => q.status === st);
        ...
        const avgDaysFor = (qs, dateField) => {
            const days = qs.map(q => {
                const sent = q.sendLogs[0]?.sentAt || q.createdAt;
                return (q[dateField].getTime() - sent.getTime()) / 86400000;
            }).filter(d => d >= 0);
        ...
        const monthAccepted = new Map();
        quotations.forEach(q => {
            const key = `${q.createdAt.getFullYear()}-${String(q.createdAt.getMonth() + 1).padStart(2, '0')}`;
            if (sentStatuses.includes(q.status))
                monthSent.set(key, (monthSent.get(key) || 0) + 1);
            if (q.status === 'ACCEPTED')
                monthAccepted.set(key, (monthAccepted.get(key) || 0) + 1);
        });
        const months = [...new Set([...monthSent.keys(), ...monthAccepted.keys()])].sort();
        const sentVsAccepted = {
    ...
    async drillDown(params) {
        const where = { tenantId: params.filters?.tenantId, createdAt: { gte: params.dateFrom, lte: params.dateTo } };
...
exports.QuotationSummaryReport = QuotationSummaryReport;
exports.QuotationSummaryReport = QuotationSummaryReport = __decorate([
...
//# sourceMappingURL=quotation-summary.report.js.map