...
let RevenueReport = class RevenueReport {
    constructor(prisma, drillDownSvc, resolver) {
        ...
        this.code = 'REVENUE';
        this.name = 'Revenue Analysis';
        this.category = 'SALES';
        this.description = 'Revenue breakdown by month, source, and industry with deal size distribution';
    ...
    async generate(params) {
        ...
        const sizeDistribution = ranges.map(r => wonLeads.filter(l => { const v = Number(l.expectedValue || 0); return v >= r.min && v < r.max; }).length);
        const sizePieChart = {
        ...
        const timelineRows = sortedMonths.map(month => {
            const mLeads = wonLeads.filter(l => `${l.updatedAt.getFullYear()}-${String(l.updatedAt.getMonth() + 1).padStart(2, '0')}` === month);
    ...
    getLeadSource(lead) {
        const sourceFilter = lead.filters?.find((f) => f.lookupValue?.lookup?.category === 'LEAD_SOURCE');
