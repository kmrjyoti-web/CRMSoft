...
exports.QuotationAgingReport = void 0;
...
let QuotationAgingReport = class QuotationAgingReport {
    constructor(prisma, drillDownSvc) {
        this.prisma = prisma;
        this.drillDownSvc = drillDownSvc;
        this.code = 'QUOTATION_AGING';
        this.name = 'Quotation Aging Report';
        this.category = 'QUOTATION';
        this.description = 'Tracks pending quotations by age buckets with recommended actions and expiry alerts';
    ...
    async generate(params) {
        ...
        const where = { tenantId: params.tenantId, status: { in: PENDING_STATUSES } };
        if (params.userId)
        ...
        for (const q of quotations) {
            ...
            if (idx >= 0) {
                bucketCounts[idx]++;
                bucketValues[idx] += Number(q.totalAmount);
            }
        }
        ...
        const isExpiring = (q) => q.validUntil && q.validUntil.getTime() <= sevenDaysFromNow.getTime() && q.validUntil.getTime() >= now.getTime();
        const summary = [
    ...
    async drillDown(params) {
        ...
        const where = { tenantId: params.filters?.tenantId, status: { in: PENDING_STATUSES } };
        if (bucket) {
...
exports.QuotationAgingReport = QuotationAgingReport;
exports.QuotationAgingReport = QuotationAgingReport = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService, drill_down_service_1.DrillDownService])
], QuotationAgingReport);
//# sourceMappingURL=quotation-aging.report.js.map