...
let RevenueAnalyticsService = class RevenueAnalyticsService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getRevenueAnalytics(params) {
        ...
        const ranges = [
            ...
            { range: '₹50K - ₹2L', min: 50000, max: 200000 },
            { range: '₹2L - ₹10L', min: 200000, max: 1000000 },
            { range: '₹10L+', min: 1000000, max: Infinity },
        ];
    ...
    async getLeadSourceAnalysis(params) {
        ...
        for (const l of leads) {
            const src = l.contact?.rawContacts?.[0]?.source || 'UNKNOWN';
            if (!bySource[src])
                bySource[src] = { total: 0, won: 0, lost: 0, active: 0, revenue: 0, closeDays: [] };
            bySource[src].total++;
            if (l.status === 'WON') {
                bySource[src].won++;
    ...
    async getLostReasonAnalysis(params) {
...
exports.RevenueAnalyticsService = RevenueAnalyticsService = __decorate([
