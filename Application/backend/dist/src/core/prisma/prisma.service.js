...
const global_reference_client_1 = require(".prisma/global-reference-client");
...
let PrismaService = PrismaService_1 = class PrismaService {
    constructor(tenantContext) {
        ...
        const logLevel = isProd ? ['error'] : ['warn', 'error'];
        this._identity = new identity_client_1.PrismaClient({
            log: logLevel,
            datasources: { db: { url: withPoolLimit(process.env.IDENTITY_DATABASE_URL || process.env.DATABASE_URL || '') } },
        });
        this._platform = new platform_client_1.PrismaClient({
            log: logLevel,
            datasources: { db: { url: withPoolLimit(process.env.PLATFORM_DATABASE_URL || process.env.DATABASE_URL || '') } },
        });
        this._globalWorking = new working_client_1.PrismaClient({
            log: logLevel,
            datasources: { db: { url: withPoolLimit(process.env.GLOBAL_WORKING_DATABASE_URL || process.env.DATABASE_URL || '') } },
        });
        this._globalReference = new global_reference_client_1.PrismaClient({
            log: logLevel,
            datasources: { db: { url: withPoolLimit(process.env.GLOBAL_REFERENCE_DATABASE_URL || process.env.DATABASE_URL || '') } },
        });
        this._demo = new demo_client_1.PrismaClient({
            log: logLevel,
            datasources: { db: { url: withPoolLimit(process.env.DEMO_DATABASE_URL || '') } },
        });
    }
    async onModuleInit() {
        this._globalWorking.$use((0, soft_delete_middleware_1.createSoftDeleteMiddleware)());
        this._demo.$use((0, soft_delete_middleware_1.createSoftDeleteMiddleware)());
        const results = await Promise.allSettled([
            this._identity.$connect(),
            this._platform.$connect(),
            this._globalWorking.$connect(),
            this._globalReference.$connect(),
            this._demo.$connect(),
        ]);
        const names = ['identity', 'platform', 'working', 'globalReference', 'demo'];
        results.forEach((r, i) => {
            if (r.status === 'rejected') {
                this.logger.warn(`[DB:${names[i]}] Could not connect at startup — will retry on first query. ` +
                    `Reason: ${r.reason?.message ?? r.reason}`);
            }
        });
        const ok = results.filter((r) => r.status === 'fulfilled').length;
        this.logger.log(`Database clients ready: ${ok}/5 connected at startup`);
    }
    async onModuleDestroy() {
        await Promise.all([
            ...
            this._globalReference.$disconnect(),
            this._demo.$disconnect(),
        ]);
    ...
    async getWorkingClient(tenantId) {
        const tenant = await this._identity.tenant.findUnique({
            where: { id: tenantId },
            select: { databaseUrl: true, hasDedicatedDb: true },
        }).catch(() => null);
        if (tenant?.hasDedicatedDb && tenant?.databaseUrl) {
            if (!this._tenantClients.has(tenantId)) {
                const client = new working_client_1.PrismaClient({
                    datasources: { db: { url: tenant.databaseUrl } },
                });
                client.$use((0, soft_delete_middleware_1.createSoftDeleteMiddleware)());
    ...
    async getWorkingOrDemoClient(tenantId) {
        const tenant = await this._identity.tenant.findUnique({
            where: { id: tenantId },
            select: { status: true },
        }).catch(() => null);
        ...
        return this.getWorkingClient(tenantId);
    }
    get $queryRaw() {
...
exports.PrismaService = PrismaService = PrismaService_1 = __decorate([
    ...
    __param(0, (0, common_1.Inject)(tenant_context_service_1.TenantContextService)),
