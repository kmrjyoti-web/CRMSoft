...
{
    private readonly logger = new Logger(GetProductPricingHandler.name);
  ...
  async execute(query: GetProductPricingQuery) {
    try {
      ...
      this.logger.error(`GetProductPricingHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
