...
describe('PaymentReminderService', () => {
  ...
  describe('error cases', () => {
    it('should propagate DB error from paymentReminder.createMany', async () => {
      mockPrisma.paymentReminder.createMany.mockRejectedValue(new Error('Insert failed'));
      await expect(service.scheduleReminders('t1', 'inv1', new Date())).rejects.toThrow('Insert failed');
    });

    it('should return empty array when no due reminders exist', async () => {
      mockPrisma.paymentReminder.findMany.mockResolvedValue([]);
      const results = await service.processDueReminders('t1');
      expect(results).toHaveLength(0);
      expect(mockPrisma.paymentReminder.update).not.toHaveBeenCalled();
    });

    it('should return 0 when no pending reminders to cancel', async () => {
      mockPrisma.paymentReminder.deleteMany.mockResolvedValue({ count: 0 });
      const count = await service.cancelReminders('t1', 'inv-none');
      expect(count).toBe(0);
    });
  });
});
