    ...
    switch (entityType) {
      case 'CONTACT': {
        ...
        records = contacts.map((c) => {
          const comms = (c.communications || []).map((cm: any) => `${cm.type}: ${cm.value}`).join(', ');
          const emails = (c.communications || []).filter((cm: any) => cm.type === 'EMAIL').map((cm: any) => cm.value).join(', ');
          const phones = (c.communications || []).filter((cm: any) => cm.type === 'PHONE' || cm.type === 'MOBILE').map((cm: any) => cm.value).join(', ');
          return {
