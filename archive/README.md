...
These directories are NOT archived because CI/CD workflows have hardcoded references to
them (`apps-backend/api/**` triggers in `.github/workflows/ci-backend.yml`).
They will be archived after CI workflows are updated to reference `apps/backend/api`.
See `docs/v6-restructure/` for migration timeline.
