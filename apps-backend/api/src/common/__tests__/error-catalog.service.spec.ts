...
describe('ErrorCatalogService', () => {
  ...
  const mockErrorCatalog = {
    findMany: jest.fn().mockResolvedValue(mockEntries),
  };
  const mockPrisma = { platform: { errorCatalog: mockErrorCatalog } };
  ...
  beforeEach(async () => {
    ...
    jest.clearAllMocks();
    mockErrorCatalog.findMany.mockResolvedValue(mockEntries);
  });

  it('should load cache on init', async () => {
    await service.onModuleInit();
    expect(mockErrorCatalog.findMany).toHaveBeenCalledWith({
      where: { isActive: true },
    });
  });
  ...
  it('should refresh cache and return count', async () => {
    ...
    expect(count).toBe(4);
    expect(mockErrorCatalog.findMany).toHaveBeenCalledTimes(2); // init + refresh
  });

  it('should handle cache refresh failure gracefully', async () => {
    await service.onModuleInit();
    mockErrorCatalog.findMany.mockRejectedValueOnce(new Error('DB down'));
  ...
  it('should auto-refresh when cache is stale', async () => {
    ...
    expect(mockErrorCatalog.findMany).toHaveBeenCalledTimes(2);
  });
});
