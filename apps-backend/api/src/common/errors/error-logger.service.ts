    ...
    try {
      const created = await this.prisma.platform.errorLog.create({
        data: {
          ...
          module: entry.module,
          requestBody: entry.requestBody ? ErrorLoggerService.sanitizeBody(entry.requestBody) as any : undefined,
          queryParams: entry.queryParams as any,
          metadata: entry.metadata as any,
          requestHeaders: entry.requestHeaders ? ErrorLoggerService.sanitizeHeaders(entry.requestHeaders) as any : undefined,
