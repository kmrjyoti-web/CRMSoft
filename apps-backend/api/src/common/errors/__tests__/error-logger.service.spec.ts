...
describe('ErrorLoggerService', () => {
  ...
  const mockPrisma = { platform: { errorLog: mockErrorLog } };
  ...
  describe('log', () => {
    ...
    it('should NOT persist 4xx errors by default', () => {
      ...
      expect(mockErrorLog.create).not.toHaveBeenCalled();
    });
    ...
    it('should persist special error codes regardless of status', async () => {
      ...
      expect(mockErrorLog.create).toHaveBeenCalled();
    });
    ...
    it('should not throw when DB persist fails', async () => {
      mockErrorLog.create.mockRejectedValue(new Error('DB down'));
  ...
  describe('getByTraceId', () => {
    it('should query by requestId', async () => {
      mockErrorLog.findMany.mockResolvedValue([{ id: '1' }]);

      const result = await service.getByTraceId('trace-123');

      expect(mockErrorLog.findMany).toHaveBeenCalledWith({
  ...
  describe('getRecent', () => {
    it('should return paginated results', async () => {
      mockErrorLog.findMany.mockResolvedValue([]);
      mockErrorLog.count.mockResolvedValue(0);
  ...
  describe('getStats', () => {
    it('should return grouped error stats', async () => {
      mockErrorLog.groupBy.mockResolvedValue([
        { errorCode: 'NOT_FOUND', _count: { id: 5 } },
        { errorCode: 'INTERNAL_ERROR', _count: { id: 3 } },
      ]);
      mockErrorLog.count.mockResolvedValue(8);
