 ...
 * Layer Bonus: Trade-specific field definitions for common CRM entities.
 *
 * Usage:
 *   const schema = getVerticalSchema('CONTACT', tenant.businessType);
 *   const errors = validateVerticalData(data, schema);
 *
 * All labels include Hindi translations (labelHi) for Indian market.
 */
...
export const VERTICAL_SCHEMAS: Record<EntityKey, Record<string, VerticalSchema>> = {
  // ─── CONTACT ───────────────────────────────────────────────────────────────
  CONTACT: {
    SOFTWARE_VENDOR: {
      fields: [
...
/**
 * Validate a verticalData record against its schema.
 * Returns array of error messages (empty = valid).
 */
export function validateVerticalData(
  ...
  for (const field of schema.fields) {
    ...
    if (!isEmpty && field.type === 'select' && field.options && !field.options.includes(value as string)) {
