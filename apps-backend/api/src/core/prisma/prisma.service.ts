...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  private _globalReference: GlobalReferenceClient;
  private _demo: DemoClient;
  ...
  /** GlobalReferenceDB — ISO countries, states, cities, system lookups */
  get globalReference(): GlobalReferenceClient {
    return this._globalReference;
  }

  /**
   * DemoDB — WorkingDB schema mirror, isolated physical DB for demo tenants.
   * Tenants with status='DEMO' have their CRM data stored here.
   * Use getWorkingOrDemoClient(tenantId) in handlers for automatic routing.
   */
  get demo(): DemoClient {
    return this._demo;
  }
  ...
  /**
   * Returns DemoDB client for DEMO tenants, WorkingDB client for all others.
   * Use this in CQRS handlers instead of direct prisma.working access
   * when the feature should be available to demo tenants.
   *
   * @example
   *   const db = await this.prisma.getWorkingOrDemoClient(cmd.tenantId);
  ...
  async getWorkingOrDemoClient(tenantId: string): Promise<WorkingClient | DemoClient> {
    ...
    if ((tenant as any)?.status === 'DEMO') {
      return this._demo;
    }

    // Dedicated per-tenant DB takes precedence over globalWorking for non-demo tenants
