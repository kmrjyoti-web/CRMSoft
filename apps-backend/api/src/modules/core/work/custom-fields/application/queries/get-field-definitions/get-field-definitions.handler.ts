...
{
    private readonly logger = new Logger(GetFieldDefinitionsHandler.name);
  ...
  async execute(query: GetFieldDefinitionsQuery) {
    try {
      ...
      this.logger.error(`GetFieldDefinitionsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
