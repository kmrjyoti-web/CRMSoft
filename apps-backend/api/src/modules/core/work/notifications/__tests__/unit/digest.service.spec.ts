...
describe('DigestService', () => {
  ...
  describe('error cases', () => {
    it('should skip regroupNotifications when groupBy returns empty array', async () => {
      prisma.notification.groupBy.mockResolvedValue([]);
      await service.regroupNotifications();
      expect(prisma.notification.findMany).not.toHaveBeenCalled();
    ...
    it('should not call updateMany when group has only one notification', async () => {
      prisma.notification.groupBy.mockResolvedValue([
        { recipientId: 'u-1', groupKey: 'solo-group', _count: 2 },
      ]);
      prisma.notification.findMany.mockResolvedValue([
        { id: 'n-solo', recipientId: 'u-1', groupKey: 'solo-group', status: 'UNREAD' },
      ]);
      await service.regroupNotifications();
      expect(prisma.notification.update).not.toHaveBeenCalled();
    ...
    it('should return empty when no subscribed users exist', async () => {
