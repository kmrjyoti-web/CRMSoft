...
describe('GetEntityTimeline', () => {
  ...
  describe('error cases', () => {
    it('should return empty timeline when no logs exist for entity', async () => {
      prisma.auditLog.findMany.mockResolvedValue([]);
      prisma.auditLog.count.mockResolvedValue(0);
      const result = await handler.execute({ entityType: 'LEAD', entityId: 'lead-999' } as any);
      expect(result.timeline).toEqual([]);
      expect(result.meta.total).toBe(0);
    });

    it('should propagate DB error from auditLog.findMany', async () => {
      prisma.auditLog.findMany.mockRejectedValue(new Error('DB timeout'));
      await expect(
        handler.execute({ entityType: 'LEAD', entityId: 'lead-1' } as any),
      ).rejects.toThrow('DB timeout');
    });

    it('should return empty timeline when page is beyond total records', async () => {
      prisma.auditLog.findMany.mockResolvedValue([]);
      prisma.auditLog.count.mockResolvedValue(3);
      const result = await handler.execute({
        entityType: 'LEAD', entityId: 'lead-1', page: 99, limit: 10,
      } as any);
      expect(result.timeline).toEqual([]);
      expect(result.meta.total).toBe(3);
    });
