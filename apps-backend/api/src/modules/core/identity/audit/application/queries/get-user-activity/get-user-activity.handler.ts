...
export class GetUserActivityHandler implements IQueryHandler<GetUserActivityQuery> {
    private readonly logger = new Logger(GetUserActivityHandler.name);
  ...
  async execute(query: GetUserActivityQuery) {
    try {
      ...
      this.logger.error(`GetUserActivityHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
