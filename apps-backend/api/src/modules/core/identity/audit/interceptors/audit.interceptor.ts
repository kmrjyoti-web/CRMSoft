...
export class AuditInterceptor implements NestInterceptor {
  ...
  private extractEntityId(response: any): string | null {
    ...
    return (data?.id as string) || (response?.id as string) || ((data?.data as Record<string, unknown>)?.id as string) || null;
  }

  private extractEntityFromResponse(response: any): Record<string, unknown> | null {
    const data = response?.data as Record<string, unknown> | undefined;
    return data?.data as Record<string, unknown> || data || response || null;
  }
}
