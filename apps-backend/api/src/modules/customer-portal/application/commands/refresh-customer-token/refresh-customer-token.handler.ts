...
export class RefreshCustomerTokenHandler implements ICommandHandler<RefreshCustomerTokenCommand> {
    private readonly logger = new Logger(RefreshCustomerTokenHandler.name);
  ...
  async execute(command: RefreshCustomerTokenCommand) {
    try {
      ...
      this.logger.error(`RefreshCustomerTokenHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
