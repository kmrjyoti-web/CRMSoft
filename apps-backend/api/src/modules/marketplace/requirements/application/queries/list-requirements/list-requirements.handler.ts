...
export class ListRequirementsHandler implements IQueryHandler<ListRequirementsQuery> {
    private readonly logger = new Logger(ListRequirementsHandler.name);
  ...
  async execute(query: ListRequirementsQuery) {
    try {
      ...
      this.logger.error(`ListRequirementsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
