...
export class GetTestDashboardHandler implements IQueryHandler<GetTestDashboardQuery> {
    private readonly logger = new Logger(GetTestDashboardHandler.name);

  constructor(private readonly prisma: PrismaService) {}

  async execute(query: GetTestDashboardQuery): Promise<Record<string, unknown>> {
    try {
      ...
      this.logger.error(`GetTestDashboardHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
