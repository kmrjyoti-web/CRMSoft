...
export class GetWorkflowVisualHandler implements IQueryHandler<GetWorkflowVisualQuery> {
    private readonly logger = new Logger(GetWorkflowVisualHandler.name);
  ...
  async execute(query: GetWorkflowVisualQuery) {
    try {
      ...
      this.logger.error(`GetWorkflowVisualHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
