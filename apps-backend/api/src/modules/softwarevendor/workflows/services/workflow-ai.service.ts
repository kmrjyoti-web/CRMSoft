...
const TRIGGER_PATTERNS: { keywords: string[]; template: NodeTemplate }[] = [
  {
...
const CONDITION_PATTERNS: { keywords: string[]; template: NodeTemplate }[] = [
  {
...
const ACTION_PATTERNS: { keywords: string[]; template: NodeTemplate }[] = [
  {
...
const DELAY_PATTERNS: { keywords: string[]; template: NodeTemplate; extractDelay: (text: string) => Record<string, any> }[] = [
  {
    ...
    extractDelay: (text: string) => {
      const hourMatch = text.match(/(\d+)\s*hours?/i);
      const dayMatch = text.match(/(\d+)\s*days?/i);
      const minMatch = text.match(/(\d+)\s*minutes?/i);
      if (hourMatch) return { duration: parseInt(hourMatch[1], 10), unit: 'hours' };
      if (dayMatch) return { duration: parseInt(dayMatch[1], 10), unit: 'days' };
      if (minMatch) return { duration: parseInt(minMatch[1], 10), unit: 'minutes' };
...
export class WorkflowAiService {
  generateFromPrompt(prompt: string, _context?: any): AiGenerateResult {
    ...
    // Default trigger if none matched
    ...
    // Build ordered list: triggers → delays → conditions → actions → end
    const orderedTemplates: NodeTemplate[] = [
      ...triggers,
      ...delays,
      ...conditions,
      ...actions,
    ...
    // Handle condition branches — shift "true" path actions down
  ...
  private matchPatterns(
    text: string,
    patterns: { keywords: string[]; template: NodeTemplate }[],
  ): NodeTemplate[] {
    const matched: NodeTemplate[] = [];
    const usedSubTypes = new Set<string>();

    for (const pattern of patterns) {
      if (usedSubTypes.has(pattern.template.nodeSubType + pattern.template.label)) continue;
  ...
  private matchActions(text: string): NodeTemplate[] {
    const matched: NodeTemplate[] = [];
    const usedLabels = new Set<string>();

    for (const pattern of ACTION_PATTERNS) {
      if (usedLabels.has(pattern.template.label)) continue;
      const hit = pattern.keywords.some((kw) => text.includes(kw));
      if (hit) {
        matched.push({ ...pattern.template });
    ...
    // Deduplicate: if we have "Send Notification" and a more specific one like "Send Email", drop generic
    if (matched.length > 1) {
      const hasSpecific = matched.some(
        (m) => m.nodeSubType === 'action_send_email' || m.nodeSubType === 'action_send_whatsapp',
      );
      if (hasSpecific) {
  ...
  private matchDelays(text: string): NodeTemplate[] {
    const matched: NodeTemplate[] = [];
    ...
    for (const pattern of DELAY_PATTERNS) {
      ...
      if (hit) {
        ...
        break; // Only one delay
      }
    }
    return matched;
  }

  private generateNodeDescription(template: NodeTemplate): string {
  ...
  private buildSuggestedName(triggers: NodeTemplate[], actions: NodeTemplate[]): string {
