...
export class UpdatePriceListHandler implements ICommandHandler<UpdatePriceListCommand> {
    private readonly logger = new Logger(UpdatePriceListHandler.name);
  ...
  async execute(cmd: UpdatePriceListCommand) {
    try {
      ...
      this.logger.error(`UpdatePriceListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
