...
export class UpdateSavedFilterHandler implements ICommandHandler<UpdateSavedFilterCommand> {
    private readonly logger = new Logger(UpdateSavedFilterHandler.name);
  ...
  async execute(cmd: UpdateSavedFilterCommand) {
    try {
      ...
      this.logger.error(`UpdateSavedFilterHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
