...
export class GetProfileListHandler implements IQueryHandler<GetProfileListQuery> {
    private readonly logger = new Logger(GetProfileListHandler.name);
  ...
  async execute(query: GetProfileListQuery) {
    try {
      ...
      this.logger.error(`GetProfileListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
