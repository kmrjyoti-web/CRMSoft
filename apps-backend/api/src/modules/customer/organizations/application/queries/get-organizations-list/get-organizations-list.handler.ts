...
export class GetOrganizationsListHandler implements IQueryHandler<GetOrganizationsListQuery> {
    private readonly logger = new Logger(GetOrganizationsListHandler.name);
  ...
  async execute(query: GetOrganizationsListQuery) {
    try {
      ...
      this.logger.error(`GetOrganizationsListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
