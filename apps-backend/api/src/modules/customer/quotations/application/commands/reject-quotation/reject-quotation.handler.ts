...
export class RejectQuotationHandler implements ICommandHandler<RejectQuotationCommand> {
    private readonly logger = new Logger(RejectQuotationHandler.name);
  ...
  async execute(cmd: RejectQuotationCommand) {
    try {
      ...
      this.logger.error(`RejectQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
