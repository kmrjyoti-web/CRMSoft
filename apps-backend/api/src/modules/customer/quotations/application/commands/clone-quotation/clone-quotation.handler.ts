...
export class CloneQuotationHandler implements ICommandHandler<CloneQuotationCommand> {
    private readonly logger = new Logger(CloneQuotationHandler.name);
  ...
  async execute(cmd: CloneQuotationCommand) {
    try {
      ...
      this.logger.error(`CloneQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
