...
export class UpdateQuotationHandler implements ICommandHandler<UpdateQuotationCommand> {
    private readonly logger = new Logger(UpdateQuotationHandler.name);
  ...
  async execute(cmd: UpdateQuotationCommand) {
    try {
      ...
      this.logger.error(`UpdateQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
