...
export class DrillDownService {
   ...
   */
  async getActivities(where: Record<string, unknown>, page: number, limit: number): Promise<DrillDownResult> {
    const skip = (page - 1) * limit;
    const [records, total] = await Promise.all([
   ...
   */
  async getDemos(where: Record<string, unknown>, page: number, limit: number): Promise<DrillDownResult> {
    const skip = (page - 1) * limit;
    const [records, total] = await Promise.all([
   ...
   */
  async getContacts(where: Record<string, unknown>, page: number, limit: number): Promise<DrillDownResult> {
    const skip = (page - 1) * limit;
    const [records, total] = await Promise.all([
