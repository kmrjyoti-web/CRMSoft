...
{
    private readonly logger = new Logger(GetEffectivePriceHandler.name);
  ...
  async execute(query: GetEffectivePriceQuery) {
    try {
      ...
      this.logger.error(`GetEffectivePriceHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
