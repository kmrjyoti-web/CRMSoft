...
(prisma as any).working = prisma;
  ...
  describe('error cases', () => {
    it('should propagate DB error from product.findMany', async () => {
      prisma.product.findMany.mockRejectedValue(new Error('DB timeout'));
      await expect(handler.execute(new ListProductsQuery(1, 10, 'createdAt', 'desc')))
    ...
    it('should return empty result when no products match', async () => {
      ...
      const result = await handler.execute(new ListProductsQuery(1, 10, 'createdAt', 'desc', 'nonexistent'));
      expect(result.data).toHaveLength(0);
      expect(result.total).toBe(0);
    });

    it('should calculate skip correctly for page 2', async () => {
      await handler.execute(new ListProductsQuery(2, 10, 'createdAt', 'desc'));
      expect(prisma.product.findMany).toHaveBeenCalledWith(
