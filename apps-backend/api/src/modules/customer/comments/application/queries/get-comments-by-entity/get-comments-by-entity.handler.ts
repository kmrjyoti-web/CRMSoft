...
export class GetCommentsByEntityHandler implements IQueryHandler<GetCommentsByEntityQuery> {
    private readonly logger = new Logger(GetCommentsByEntityHandler.name);
  ...
  async execute(query: GetCommentsByEntityQuery) {
    try {
      ...
      this.logger.error(`GetCommentsByEntityHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
