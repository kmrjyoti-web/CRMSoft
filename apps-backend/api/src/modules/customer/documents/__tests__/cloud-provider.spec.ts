...
(prisma as any).working = prisma;
  ...
  describe('error cases', () => {
    it('should propagate DB error from cloudConnection.update on reconnect', async () => {
      prisma.cloudConnection.update.mockRejectedValue(new Error('DB write failed'));
      await expect(service.connectProvider('u1', 'GOOGLE_DRIVE' as any, 'token', 'refresh'))
        .rejects.toThrow('DB write failed');
    });

    it('should return empty array when user has no connections', async () => {
      prisma.cloudConnection.findMany.mockResolvedValue([]);
      const result = await service.getConnections('u1');
      expect(result).toHaveLength(0);
    });

    it('should propagate DB error from cloudConnection.updateMany on disconnect', async () => {
      prisma.cloudConnection.updateMany.mockRejectedValue(new Error('Update failed'));
      await expect(service.disconnectProvider('u1', 'GOOGLE_DRIVE' as any))
