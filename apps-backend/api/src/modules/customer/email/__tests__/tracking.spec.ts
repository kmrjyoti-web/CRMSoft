...
(prisma as any).working = prisma;
  ...
  describe('edge cases', () => {
    it('should inject pixel into empty HTML string', () => {
      const result = service.injectOpenPixel('', 'pixel-empty');
      expect(result).toContain('/email/track/open/pixel-empty');
    });

    it('should not rewrite links when HTML has no anchor tags', () => {
      const html = '<p>No links here</p>';
      const result = service.rewriteLinks(html, 'email-no-links');
      expect(result).toBe(html);
    });

    it('should generate unique tracking pixel IDs on each call', () => {
      const id1 = service.generateTrackingPixelId();
      const id2 = service.generateTrackingPixelId();
