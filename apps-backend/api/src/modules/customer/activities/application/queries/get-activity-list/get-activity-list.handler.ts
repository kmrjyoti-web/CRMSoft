...
export class GetActivityListHandler implements IQueryHandler<GetActivityListQuery> {
    private readonly logger = new Logger(GetActivityListHandler.name);
  ...
  async execute(query: GetActivityListQuery) {
    try {
      ...
      this.logger.error(`GetActivityListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
