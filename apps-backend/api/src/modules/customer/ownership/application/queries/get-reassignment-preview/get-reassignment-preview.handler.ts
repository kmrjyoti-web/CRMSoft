...
export class GetReassignmentPreviewHandler implements IQueryHandler<GetReassignmentPreviewQuery> {
    private readonly logger = new Logger(GetReassignmentPreviewHandler.name);
  ...
  async execute(query: GetReassignmentPreviewQuery) {
    try {
      ...
      this.logger.error(`GetReassignmentPreviewHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
