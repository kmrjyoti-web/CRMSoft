...
(prisma as any).working = prisma;
  ...
  describe('edge cases', () => {
    it('should return empty calendar grid when no activities exist', async () => {
    ...
    it('should propagate DB error from activity.findMany', async () => {
      prisma.activity.findMany.mockRejectedValue(new Error('DB timeout'));
      await expect(service.getActivityHeatmap({
        dateFrom: new Date('2025-01-01'), dateTo: new Date('2025-01-31'),
      })).rejects.toThrow('DB timeout');
    });

    it('should return zeros in weekly grid when all activities are same hour', async () => {
      const sameHour = new Date('2025-01-06T10:00:00');
      prisma.activity.findMany.mockResolvedValue([
        { createdAt: sameHour, type: 'CALL' },
        { createdAt: sameHour, type: 'CALL' },
      ]);
      const result = await service.getActivityHeatmap({
        dateFrom: new Date('2025-01-01'), dateTo: new Date('2025-01-31'),
