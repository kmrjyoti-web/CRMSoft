...
(prisma as any).working = prisma;
  ...
  describe('edge cases', () => {
    it('should handle empty pipeline (no leads in any stage)', async () => {
    ...
    it('should handle zero total leads in funnel without division errors', async () => {
    ...
    it('should propagate DB error from lead.groupBy', async () => {
      prisma.lead.groupBy.mockRejectedValue(new Error('Query failed'));
      await expect(service.getSalesPipeline({})).rejects.toThrow('Query failed');
    });
  });
});
