...
export class GetContactByIdHandler implements IQueryHandler<GetContactByIdQuery> {
    private readonly logger = new Logger(GetContactByIdHandler.name);
  ...
  async execute(query: GetContactByIdQuery) {
    try {
      ...
      this.logger.error(`GetContactByIdHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
