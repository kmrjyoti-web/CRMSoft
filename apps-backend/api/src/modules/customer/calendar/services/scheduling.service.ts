...
export class SchedulingService {
  ...
  async createEvent(dto: any, userId: string, tenantId: string, roleLevel: number) {
    ...
    if (dto.participants?.length) {
      await this.prisma.working.eventParticipant.createMany({
        data: dto.participants.map((p: Record<string, unknown>) => ({
  ...
  async updateEvent(id: string, dto: any, userId: string, tenantId: string) {
    ...
    for (const field of trackableFields) {
      if (dto[field] !== undefined && dto[field] !== (event as unknown)[field]) {
        changes.push({ field, oldValue: String((event as unknown)[field] ?? ''), newValue: String(dto[field]) });
