...
// ---------------------------------------------------------------------------
// Realistic Indian names, companies, designations & departments
// ---------------------------------------------------------------------------
...
const COMPANIES = [
  ...
  null, null, null, null, null, // ~5% without company
];
...
const DESIGNATIONS = [
  ...
  null, null, null, null, null, // ~12% without designation
];
...
const DEPARTMENTS = [
  ...
  null, null, null, null, // ~17% without department
];

const SOURCES: ('MANUAL' | 'BULK_IMPORT' | 'WEB_FORM' | 'REFERRAL' | 'API')[] = [
  'MANUAL', 'MANUAL', 'MANUAL',        // 30% manual
  'BULK_IMPORT', 'BULK_IMPORT',         // 20% bulk
  'WEB_FORM', 'WEB_FORM', 'WEB_FORM',  // 30% web form
  'REFERRAL',                           // 10% referral
  'API',                                // 10% API
];

const STATUSES: ('RAW' | 'VERIFIED' | 'REJECTED' | 'DUPLICATE')[] = [
  'RAW', 'RAW', 'RAW', 'RAW', 'RAW',       // 50% RAW
  'VERIFIED', 'VERIFIED',                    // 20% VERIFIED
...
const NOTES_TEMPLATES = [
  ...
  null, null, null, null, null, // ~25% without notes
];
...
function generateNote(): string | null {
...
export async function seedRawContacts(
  prisma: PrismaClient,
  adminUserId: string,
  tenantId: string,
  count = 1000,
  ...
  for (let batch = 0; batch < Math.ceil(toCreate / batchSize); batch++) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      const createdAt = randomDate(365, 0); // within last year
      ...
      // Generate 1-3 communications per contact
      const commCount = randomInt(1, 3);
      const comms: { type: string; value: string; priorityType: string; isPrimary: boolean }[] = [];
      ...
      if (commCount >= 3) {
        // Add secondary phone or WhatsApp
    ...
    // Create raw contacts one by one (with communications)
