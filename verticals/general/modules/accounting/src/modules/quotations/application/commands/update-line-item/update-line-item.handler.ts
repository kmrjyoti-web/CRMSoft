...
export class UpdateLineItemHandler implements ICommandHandler<UpdateLineItemCommand> {
    private readonly logger = new Logger(UpdateLineItemHandler.name);
  ...
  async execute(cmd: UpdateLineItemCommand) {
    try {
      ...
      this.logger.error(`UpdateLineItemHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
