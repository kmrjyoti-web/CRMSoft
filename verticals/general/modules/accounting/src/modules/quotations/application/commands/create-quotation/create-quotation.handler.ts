...
export class CreateQuotationHandler implements ICommandHandler<CreateQuotationCommand> {
    private readonly logger = new Logger(CreateQuotationHandler.name);
  ...
  async execute(cmd: CreateQuotationCommand) {
    try {
      ...
      this.logger.error(`CreateQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
