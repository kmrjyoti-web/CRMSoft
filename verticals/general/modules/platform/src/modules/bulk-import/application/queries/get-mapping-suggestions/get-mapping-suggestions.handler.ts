...
export class GetMappingSuggestionsHandler implements IQueryHandler<GetMappingSuggestionsQuery> {
    private readonly logger = new Logger(GetMappingSuggestionsHandler.name);
  ...
  async execute(query: GetMappingSuggestionsQuery) {
    try {
      ...
      this.logger.error(`GetMappingSuggestionsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
