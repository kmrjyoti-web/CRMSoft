...
import { TaskAssignmentService } from '../../tasks/application/services/task-assignment.service';
...
export class CalendarVisibilityService {
   ...
   */
  async buildWhereClause(ctx: CalendarVisibilityContext): Promise<any> {
   ...
   */
  async buildCalendarEventWhere(ctx: CalendarVisibilityContext): Promise<any> {
    const { userId, roleLevel, tenantId } = ctx;
