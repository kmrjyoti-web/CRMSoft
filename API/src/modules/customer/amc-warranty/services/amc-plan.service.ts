...
export class AMCPlanService {
  ...
  async findAll(tenantId: string, filters?: { industryCode?: string; planTier?: string }) {
    return this.prisma.aMCPlanTemplate.findMany({
      where: {
  ...
  async findById(id: string) {
    const plan = await this.prisma.aMCPlanTemplate.findUnique({ where: { id } });
    if (!plan) throw new NotFoundException('AMC plan not found');
    return plan;
  }

  async create(tenantId: string, dto: any) {
    const existing = await this.prisma.aMCPlanTemplate.findFirst({ where: { tenantId, code: dto.code } });
    if (existing) throw new ConflictException(`AMC plan with code ${dto.code} already exists`);
    return this.prisma.aMCPlanTemplate.create({
      data: { ...dto, tenantId, isSystemTemplate: false },
    });
  }

  async update(tenantId: string, id: string, dto: any) {
    const plan = await this.prisma.aMCPlanTemplate.findFirst({ where: { id, tenantId, isSystemTemplate: false } });
    if (!plan) throw new NotFoundException('Plan not found or cannot edit system plan');
    return this.prisma.aMCPlanTemplate.update({ where: { id }, data: dto });
  }

  async importSystemPlan(tenantId: string, systemPlanId: string) {
    const systemPlan = await this.prisma.aMCPlanTemplate.findFirst({
    ...
    const existing = await this.prisma.aMCPlanTemplate.findFirst({ where: { tenantId, code: newCode } });
