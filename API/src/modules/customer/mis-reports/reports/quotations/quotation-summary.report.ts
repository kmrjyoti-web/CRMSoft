  ...
  async generate(params: ReportParams): Promise<ReportData> {
    ...
    const avgDaysFor = (qs: typeof quotations, dateField: 'acceptedAt' | 'rejectedAt') => {
      const days = qs.map(q => {
        ...
        return ((q as any)[dateField]!.getTime() - sent.getTime()) / 86400000;
      }).filter(d => d >= 0);
    ...
    quotations.forEach(q => {
      const uid = q.createdByUser?.id || 'unknown';
      const name = q.createdByUser ? `${q.createdByUser.firstName} ${q.createdByUser.lastName}` : 'Unknown';
