...
export class CompareTestRunsHandler implements IQueryHandler<CompareTestRunsQuery> {
  ...
  async execute(query: CompareTestRunsQuery): Promise<any> {
    ...
    const run1Map = new Map<string, any>(
      (run1.results ?? []).map((r: any) => [`${r.testType}::${r.suiteName}::${r.testName}`, r]),
    );
    const run2Map = new Map<string, any>(
      (run2.results ?? []).map((r: any) => [`${r.testType}::${r.suiteName}::${r.testName}`, r]),
    );
