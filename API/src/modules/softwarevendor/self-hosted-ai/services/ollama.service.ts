...
export class OllamaService {
  ...
  async listLocalModels(): Promise<OllamaModelInfo[]> {
    try {
      ...
      this.logger.error('Failed to list Ollama models', e.message);
      return [];
    }
  }
  ...
  async generate(modelName: string, prompt: string, systemPrompt?: string): Promise<{
    ...
    try {
      ...
      throw new BadRequestException(`Ollama generation failed: ${e.message}`);
    }
  }
  ...
  async chat(modelName: string, messages: { role: string; content: string }[]): Promise<{
    ...
    try {
      ...
      throw new BadRequestException(`Ollama chat failed: ${e.message}`);
    }
  }
  ...
  async embed(modelName: string, text: string): Promise<number[]> {
    try {
      ...
      throw new BadRequestException(`Embedding generation failed: ${e.message}`);
    }
  }
