...
export async function seedAccountMaster(prisma: PrismaClient, tenantId: string) {
  ...
  // Second pass: direct children of root groups
  const level1Defs = groupDefs.filter(
    (d) => d.parentCode && groupDefs.find((r) => r.code === d.parentCode && !r.parentCode),
  );
  ...
  // Third pass: grandchildren (children of level-1 groups like DUTIES_TAXES)
  const level2Defs = groupDefs.filter(
    (d) => d.parentCode && !groupDefs.find((r) => r.code === d.parentCode && !r.parentCode),
  );
