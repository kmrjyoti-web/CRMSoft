...
The DB Schema Auditor runs on every PR and enforces three rules:

- **Naming conventions** — all Prisma models must have `@@map`, table names must be snake_case, vertical-prefixed tables must use a valid module code
- **Cross-DB include detection** — Prisma `include:` that crosses database boundaries is forbidden; use `CrossDbResolverService` instead
- **FK orphan detection** — runtime scan for broken foreign key references (runs nightly, not in CI)

Run locally:
```bash
cd Application/backend
npm run audit:db                    # all checks
npm run audit:db -- --check=naming  # single check
npm run audit:db -- --format=json   # JSON for scripting
```

**Rule:** Any non-zero error count blocks merge. No allowlists, no exceptions. See `docs/health-reports/latest-audit.json` for the latest run.
