...
import { PujaGreeting } from "@/features/puja/components/PujaGreeting";
...
export function DashboardOverview() {
  ...
  useEffect(() => {
    pujaService.getStatus()
      .then((s) => { if (s.show && s.config) setPujaConfig(s.config); })
      .catch(() => {});
  }, []);
  ...
  return (
    <div>
      {/* Puja greeting banner — shown when religious mode is active */}
      {pujaConfig && <PujaGreeting config={pujaConfig} />}
