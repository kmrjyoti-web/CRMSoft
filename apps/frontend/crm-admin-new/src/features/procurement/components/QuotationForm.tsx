...
import { useQuotationDetail, useCreateQuotation, useUpdateQuotation } from "../hooks/useProcurement";
...
type ProcurementQuotationFormValues = z.infer<typeof quotationSchema>;
...
export function QuotationForm({ quotationId, mode = "page", panelId, onSuccess }: QuotationFormProps) {
  ...
  const { data: quotationData, isLoading } = useQuotationDetail(quotationId ?? "");
  ...
  } = useForm<ProcurementQuotationFormValues>({
    ...
    updatePanelConfig(panelId, {
      footerButtons: [
        { id: "cancel", label: "Cancel", showAs: "text", variant: "secondary", disabled: isSubmitting, onClick: () => {} },
        {
          ...
          onClick: () => {
            const formEl = document.getElementById(`sp-form-quotation-${quotationId ?? "new"}`) as HTMLFormElement | null;
  ...
  const onSubmit = async (values: ProcurementQuotationFormValues) => {
    try {
      const payload = {
        vendorId: values.vendorId,
        rfqId: values.rfqId || undefined,
      ...
      if (isEdit && quotationId) {
        ...
        else router.push(`/procurement/quotations/${quotationId}`);
      } else {
        await createMutation.mutateAsync(payload);
        toast.success("Quotation created");
        if (isPanel && onSuccess) onSuccess();
        else router.push("/procurement/quotations");
      }
    } catch {
  ...
  return (
    <div className={isPanel ? "p-4" : "p-6 max-w-5xl mx-auto"} style={{ position: "relative" }}>
      ...
      {!isPanel && (
        <PageHeader
          title={isEdit ? "Edit Purchase Quotation" : "New Purchase Quotation"}
          actions={<Button variant="outline" onClick={() => router.back()}><Icon name="arrow-left" size={16} /> Back</Button>}
        />
      )}

      <form
        id={isPanel ? `sp-form-quotation-${quotationId ?? "new"}` : undefined}
      ...
      >
        ...
        <Fieldset label="Quotation Information">
          <div className="grid grid-cols-3 gap-4">
            ...
            <Controller name="rfqId" control={control} render={({ field }) => (
              <Input label="RFQ Reference" placeholder="RFQ ID (optional)" value={field.value ?? ""} onChange={field.onChange} />
            )} />
            <Controller name="quotationDate" control={control} render={({ field }) => (
              <SmartDateInput label="Quotation Date" required value={field.value || null} onChange={(v) => field.onChange(v ?? "")} />
            )} />
            <Controller name="validUntil" control={control} render={({ field }) => (
              <SmartDateInput label="Valid Until" value={field.value || null} onChange={(v) => field.onChange(v ?? "")} />
            )} />
            <Controller name="creditDays" control={control} render={({ field }) => (
              <NumberInput label="Credit Days" value={field.value ?? null} onChange={field.onChange} min={0} precision={0} />
            )} />
            <Controller name="deliveryDays" control={control} render={({ field }) => (
              <NumberInput label="Delivery Days" value={field.value ?? null} onChange={field.onChange} min={0} precision={0} />
            )} />
          </div>
        </Fieldset>
        ...
        <div className="flex flex-wrap gap-3">
          <button type="button" onClick={() => setShowTermsModal(true)}
            ...
            {(watch("paymentTerms") || watch("deliveryTerms")) ? <span className="ml-2 text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded">Added</span> : <span className="ml-2 text-xs text-gray-400">+ Add</span>}
          </button>
        ...
        <Modal open={showTermsModal} onClose={() => setShowTermsModal(false)} title="Payment & Delivery Terms" size="md"
          ...
          <div className="space-y-4">
            <Controller name="paymentTerms" control={control} render={({ field }) => (
              <div><label className="mb-1 block text-sm font-medium text-gray-700">Payment Terms</label>
                <textarea className="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:border-blue-400 focus:outline-none"
                  value={field.value ?? ""} onChange={field.onChange} rows={3} /></div>
            )} />
            <Controller name="deliveryTerms" control={control} render={({ field }) => (
              <div><label className="mb-1 block text-sm font-medium text-gray-700">Delivery Terms</label>
                <textarea className="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:border-blue-400 focus:outline-none"
                  value={field.value ?? ""} onChange={field.onChange} rows={3} /></div>
            )} />
          </div>
