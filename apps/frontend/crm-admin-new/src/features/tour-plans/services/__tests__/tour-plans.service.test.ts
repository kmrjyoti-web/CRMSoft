...
describe("tourPlansService", () => {
  ...
  it("getAll calls GET /tour-plans with params", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith("/api/v1/tour-plans", {
  ...
  it("getById calls GET /tour-plans/:id", async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith("/api/v1/tour-plans/tp-1");
  });
  ...
  it("create sends POST /tour-plans", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith("/api/v1/tour-plans", payload);
  });

  it("update sends PUT /tour-plans/:id", async () => {
    const payload = { title: "Updated Plan" };
    await tourPlansService.update("tp-1", payload);
    expect(apiClient.put).toHaveBeenCalledWith("/api/v1/tour-plans/tp-1", payload);
  });

  it("submit sends POST /tour-plans/:id/submit", async () => {
    await tourPlansService.submit("tp-1");
    expect(apiClient.post).toHaveBeenCalledWith("/api/v1/tour-plans/tp-1/submit");
  });

  it("approve sends POST /tour-plans/:id/approve", async () => {
    await tourPlansService.approve("tp-1");
    expect(apiClient.post).toHaveBeenCalledWith("/api/v1/tour-plans/tp-1/approve");
  });
  ...
  it("reject sends POST /tour-plans/:id/reject", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith("/api/v1/tour-plans/tp-1/reject", payload);
  });
});
