...
export class GetTestResultsTreeHandler implements IQueryHandler<GetTestResultsTreeQuery> {
    private readonly logger = new Logger(GetTestResultsTreeHandler.name);
  ...
  async execute(query: GetTestResultsTreeQuery): Promise<any> {
    try {
      ...
      return Object.entries(tree).map(([testType, suites]: [string, any]) => {
        const suiteList = (Object.entries(suites) as [string, any[]][]).map(([suiteName, tests]) => ({
          suiteName,
          total: tests.length,
      ...
      this.logger.error(`GetTestResultsTreeHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
