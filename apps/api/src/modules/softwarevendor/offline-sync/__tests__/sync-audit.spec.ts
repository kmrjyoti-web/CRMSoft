...
describe('SyncAnalyticsService', () => {
  it('getDashboard aggregates device and conflict counts', async () => {
    ...
    const dashboard = await service.getDashboard() as any;
  ...
  it('getAnalytics calculates sync metrics', async () => {
    ...
    const analytics = await service.getAnalytics() as any;
  ...
  describe('edge cases', () => {
    it('getDashboard returns zero avgPendingUploads when no devices', async () => {
      const prisma = makeMockPrisma({
        syncDevice: {
          count: jest.fn().mockResolvedValue(0),
          findMany: jest.fn().mockResolvedValue([]),
      ...
      expect(dashboard.avgPendingUploads).toBe(0);
    });

    it('getAnalytics returns zeros when no sync logs exist', async () => {
      const prisma = makeMockPrisma({
        syncAuditLog: {
          findMany: jest.fn().mockResolvedValue([]),
        },
      ...
      expect(analytics.totalPulls).toBe(0);
      expect(analytics.totalPushes).toBe(0);
      expect(analytics.totalConflicts).toBe(0);
    });

    it('getAuditLog propagates DB error from syncAuditLog.findMany', async () => {
      const prisma = makeMockPrisma({
        syncAuditLog: {
      ...
      const service = new SyncAnalyticsService(prisma);
      await expect(service.getAuditLog({ page: 1, limit: 20 })).rejects.toThrow('Query failed');
    });
  });
});
