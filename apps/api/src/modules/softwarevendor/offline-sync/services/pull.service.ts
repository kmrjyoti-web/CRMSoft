...
export class PullService {
  ...
  private async findDeletedIds(
    ...
    if (config.softDeleteField === 'isActive') {
      ...
      ids.push(...deactivated.map((r: Record<string, unknown>) => r.id));
    }
    ...
    if (config.terminalStatuses.length > 0) {
      ...
      ids.push(...terminated.map((r: Record<string, unknown>) => r.id));
    }

    return [...new Set(ids)];
  }

  private stripExcludedFields(records: Record<string, unknown>[], excludeFields: string[]): Record<string, unknown>[] {
