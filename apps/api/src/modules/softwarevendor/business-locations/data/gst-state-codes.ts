export interface GstStateCode {
  code: string;  // GST state code e.g. "27"
  name: string;  // "Maharashtra"
  iso: string;   // ISO 3166-2 alpha-2 e.g. "MH"
}
...
export function getGstCodeForState(isoCode: string): string | undefined {
  return GST_STATE_CODES.find(g => g.iso === isoCode)?.code;
}
