...
export class ValidateWorkflowHandler implements ICommandHandler<ValidateWorkflowCommand> {
    private readonly logger = new Logger(ValidateWorkflowHandler.name);
  ...
  async execute(cmd: ValidateWorkflowCommand) {
    try {
      ...
      this.logger.error(`ValidateWorkflowHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
