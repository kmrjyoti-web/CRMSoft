...
export class PublishWorkflowHandler implements ICommandHandler<PublishWorkflowCommand> {
    private readonly logger = new Logger(PublishWorkflowHandler.name);
  ...
  async execute(cmd: PublishWorkflowCommand) {
    try {
      ...
      this.logger.error(`PublishWorkflowHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
