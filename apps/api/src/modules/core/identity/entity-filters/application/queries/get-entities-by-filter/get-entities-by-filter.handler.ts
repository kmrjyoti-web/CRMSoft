...
export class GetEntitiesByFilterHandler implements IQueryHandler<GetEntitiesByFilterQuery> {
    private readonly logger = new Logger(GetEntitiesByFilterHandler.name);
  ...
  async execute(query: GetEntitiesByFilterQuery) {
    try {
      ...
      if (query.matchAll) {
        ...
        return results.map((r: Record<string, unknown>) => r[config.fkField]);
      } else {
      ...
      this.logger.error(`GetEntitiesByFilterHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
