...
describe('HolidayService', () => {
  ...
  describe('error cases', () => {
    it('should return false when no holidays exist for date', async () => {
      mockPrisma.holidayCalendar.findMany.mockResolvedValue([]);
      const isHoliday = await service.isHoliday('t1', new Date());
      expect(isHoliday).toBe(false);
    });

    it('national holiday should apply regardless of state', async () => {
      mockPrisma.holidayCalendar.findMany.mockResolvedValue([
        { type: 'NATIONAL', applicableStates: [] },
      ]);
      const isHoliday = await service.isHoliday('t1', new Date(), 'KL');
      expect(isHoliday).toBe(true);
    });

    it('should propagate DB error from holidayCalendar.findMany', async () => {
      mockPrisma.holidayCalendar.findMany.mockRejectedValue(new Error('Connection refused'));
      await expect(service.isHoliday('t1', new Date(), 'MH')).rejects.toThrow('Connection refused');
    });
  });
});
