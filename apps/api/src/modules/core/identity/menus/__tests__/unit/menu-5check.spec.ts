...
describe('GetMyMenuHandler - 5-Check Visibility Chain', () => {
  ...
  beforeEach(async () => {
    mockPrisma = {
      ...
      businessTypeRegistry: { findUnique: jest.fn().mockResolvedValue(null) },
    };
  ...
  function setupDefaults() {
    ...
    mockPrisma.tenant.findUnique.mockResolvedValue({ id: 't1', businessTypeId: null });
  ...
  it('should resolve terminology key in menu names', async () => {
    mockPrisma.rolePermission.findMany.mockResolvedValue([]);
    mockPrisma.tenantModule.findMany.mockResolvedValue([]);
    mockPrisma.tenant.findUnique.mockResolvedValue({ id: 't1', businessTypeId: 'bt1' });
    mockPrisma.businessTypeRegistry.findUnique.mockResolvedValue({
      id: 'bt1',
      terminologyMap: { Contact: 'Patient', Lead: 'Appointment' },
    });
    mockPrisma.terminologyOverride.findMany.mockResolvedValue([]);
