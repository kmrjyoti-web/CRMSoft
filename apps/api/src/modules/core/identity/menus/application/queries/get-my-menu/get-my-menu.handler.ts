...
interface MenuRow {
  ...
  businessTypeApplicability: Record<string, unknown>;
...
export class GetMyMenuHandler implements IQueryHandler<GetMyMenuQuery> {
    private readonly logger = new Logger(GetMyMenuHandler.name);
  ...
  async execute(query: GetMyMenuQuery): Promise<MenuTreeItem[]> {
    try {
      ...
      this.logger.error(`GetMyMenuHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
}
