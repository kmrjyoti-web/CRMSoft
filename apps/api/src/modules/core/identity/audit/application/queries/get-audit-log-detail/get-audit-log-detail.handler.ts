...
export class GetAuditLogDetailHandler implements IQueryHandler<GetAuditLogDetailQuery> {
    private readonly logger = new Logger(GetAuditLogDetailHandler.name);
  ...
  async execute(query: GetAuditLogDetailQuery) {
    try {
      ...
      this.logger.error(`GetAuditLogDetailHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
