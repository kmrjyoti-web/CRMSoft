...
export class GetFollowUpListHandler implements IQueryHandler<GetFollowUpListQuery> {
    private readonly logger = new Logger(GetFollowUpListHandler.name);
  ...
  async execute(query: GetFollowUpListQuery) {
    try {
      ...
      this.logger.error(`GetFollowUpListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
