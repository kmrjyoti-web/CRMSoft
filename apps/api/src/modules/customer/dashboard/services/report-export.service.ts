...
export class ReportExportService {
  ...
  private async queryReportData(reportType: string, filters: any): Promise<{ columns: ColumnDef[]; data: Record<string, unknown>[] }> {
  ...
  private async generateExcel(params: {
    columns: ColumnDef[]; data: any[]; sheetName: string; title: string; filters: Record<string, unknown>;
  }): Promise<Buffer> {
  ...
  private generateCsv(columns: ColumnDef[], data: any[]): string {
    const escape = (val: Record<string, unknown>) => {
