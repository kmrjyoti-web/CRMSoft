...
(prisma as any).working = prisma;
  ...
  describe('error cases', () => {
    it('should return zero records for unsupported report type', async () => {
      const result = await service.exportReport({
        reportType: 'INVALID_REPORT' as any, format: 'CSV',
        filters: {}, exportedById: 'user-1', exportedByName: 'Alice Smith',
      });
      expect(result.recordCount).toBe(0);
    });

    it('should propagate DB error from reportExportLog.create', async () => {
      prisma.reportExportLog.create.mockRejectedValue(new Error('DB write failed'));
    ...
    it('should export with zero records when lead.findMany returns empty', async () => {
