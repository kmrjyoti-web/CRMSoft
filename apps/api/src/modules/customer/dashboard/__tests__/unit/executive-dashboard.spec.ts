...
(prisma as any).working = prisma;
  ...
  describe('error cases', () => {
    it('should propagate DB error from lead.count', async () => {
      prisma.lead.count.mockRejectedValue(new Error('DB unavailable'));
      await expect(service.getExecutiveDashboard({
        dateFrom: new Date('2025-01-01'), dateTo: new Date('2025-01-31'),
      })).rejects.toThrow('DB unavailable');
    });

    it('should handle zero total leads without division errors', async () => {
      prisma.lead.count.mockResolvedValue(0);
      const result = await service.getExecutiveDashboard({
        dateFrom: new Date('2025-01-01'), dateTo: new Date('2025-01-31'),
      });
      const convCard = result.kpiCards.find((c: any) => c.key === 'conversion_rate');
      expect(convCard).toBeDefined();
      expect((convCard as any).value).toBe(0);
    });

    it('should return empty targets in my dashboard when no salesTargets', async () => {
      prisma.salesTarget.findMany.mockResolvedValue([]);
      const result = await service.getMyDashboard('user-1');
      expect(result.myTargets).toHaveLength(0);
    });
  });
});
