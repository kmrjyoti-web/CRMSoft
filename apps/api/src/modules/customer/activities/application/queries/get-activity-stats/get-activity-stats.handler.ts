...
export class GetActivityStatsHandler implements IQueryHandler<GetActivityStatsQuery> {
    private readonly logger = new Logger(GetActivityStatsHandler.name);
  ...
  async execute(query: GetActivityStatsQuery) {
    try {
      ...
      this.logger.error(`GetActivityStatsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
