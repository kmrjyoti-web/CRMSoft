...
export class DismissReminderHandler implements ICommandHandler<DismissReminderCommand> {
    private readonly logger = new Logger(DismissReminderHandler.name);
  ...
  async execute(cmd: DismissReminderCommand) {
    try {
      ...
      this.logger.error(`DismissReminderHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
