...
export class SelectProfileHandler implements ICommandHandler<SelectProfileCommand> {
    private readonly logger = new Logger(SelectProfileHandler.name);
  ...
  async execute(cmd: SelectProfileCommand) {
    try {
      ...
      await this.prisma.working.importJob.update({
        where: { id: cmd.jobId },
        data: { fieldMapping: match.resolvedMapping, status: 'MAPPING' } as any,
      });
      ...
      this.logger.error(`SelectProfileHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }

  /** Auto-map all rows when profile is a full match */
  private async autoMapRows(jobId: string, resolvedMapping: Record<string, unknown>[], profile: any): Promise<void> {
    ...
    await this.prisma.working.importJob.update({
      where: { id: jobId },
      data: { fieldMapping: resolvedMapping, status: 'MAPPED', usedAutoMapping: true } as any,
    });
  }
}
