...
export class GetContactsDashboardHandler implements IQueryHandler<GetContactsDashboardQuery> {
    private readonly logger = new Logger(GetContactsDashboardHandler.name);
  ...
  async execute(query: GetContactsDashboardQuery) {
    try {
      ...
      this.logger.error(`GetContactsDashboardHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
