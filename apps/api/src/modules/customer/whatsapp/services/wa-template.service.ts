...
export class WaTemplateService {
  ...
  async syncFromMeta(wabaId: string): Promise<{ synced: number; added: number; updated: number }> {
    ...
    for (const mt of metaTemplates) {
      ...
      const headerComponent = mt.components?.find((c: Record<string, unknown>) => c.type === 'HEADER');
      const bodyComponent = mt.components?.find((c: Record<string, unknown>) => c.type === 'BODY');
      const footerComponent = mt.components?.find((c: Record<string, unknown>) => c.type === 'FOOTER');
      const buttonsComponent = mt.components?.find((c: Record<string, unknown>) => c.type === 'BUTTONS');
  ...
  async createOnMeta(wabaId: string, templateData: {
    ...
    return this.prisma.working.waTemplate.create({
      data: {
        ...
        sampleValues: templateData.sampleValues as any,
      },
    });
  }
