...
export class GetTourPlanStatsHandler implements IQueryHandler<GetTourPlanStatsQuery> {
    private readonly logger = new Logger(GetTourPlanStatsHandler.name);
  ...
  async execute(query: GetTourPlanStatsQuery) {
    try {
      ...
      this.logger.error(`GetTourPlanStatsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
