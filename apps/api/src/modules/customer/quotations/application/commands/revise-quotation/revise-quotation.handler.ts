...
export class ReviseQuotationHandler implements ICommandHandler<ReviseQuotationCommand> {
    private readonly logger = new Logger(ReviseQuotationHandler.name);
  ...
  async execute(cmd: ReviseQuotationCommand) {
    try {
      ...
      this.logger.error(`ReviseQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
