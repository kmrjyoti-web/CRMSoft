...
export class CreateFromTemplateHandler implements ICommandHandler<CreateFromTemplateCommand> {
    private readonly logger = new Logger(CreateFromTemplateHandler.name);
  ...
  async execute(cmd: CreateFromTemplateCommand) {
    try {
      ...
      this.logger.error(`CreateFromTemplateHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
