...
export class AiGenerateQuotationHandler implements ICommandHandler<AiGenerateQuotationCommand> {
    private readonly logger = new Logger(AiGenerateQuotationHandler.name);
  ...
  async execute(cmd: AiGenerateQuotationCommand) {
    try {
      ...
      this.logger.error(`AiGenerateQuotationHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
