...
export class ListQuotationsHandler implements IQueryHandler<ListQuotationsQuery> {
    private readonly logger = new Logger(ListQuotationsHandler.name);
  ...
  async execute(query: ListQuotationsQuery) {
    try {
      ...
      this.logger.error(`ListQuotationsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
