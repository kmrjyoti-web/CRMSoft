...
(mockPrisma as any).working = mockPrisma;
  ...
  describe('edge cases', () => {
    it('ActivitySummaryReport: returns zero totalActivities when no activities', async () => {
      const mockPrisma: any = { activity: { findMany: jest.fn().mockResolvedValue([]), count: jest.fn().mockResolvedValue(0) } };
      (mockPrisma as any).working = mockPrisma;
      const mockDrill: any = { getActivities: jest.fn() };
      const r = new ActivitySummaryReport(mockPrisma, mockDrill);
      const result = await r.generate(baseParams);
      const total = result.summary.find((m: any) => m.key === 'totalActivities');
      expect(total!.value).toBe(0);
    });

    it('ActivitySummaryReport: propagates DB error from activity.findMany', async () => {
      const mockPrisma: any = { activity: { findMany: jest.fn().mockRejectedValue(new Error('DB error')), count: jest.fn() } };
      (mockPrisma as any).working = mockPrisma;
      const mockDrill: any = { getActivities: jest.fn() };
      const r = new ActivitySummaryReport(mockPrisma, mockDrill);
      await expect(r.generate(baseParams)).rejects.toThrow('DB error');
    });

    it('ActivityHeatmapReport: returns all-zero grid when no activities', async () => {
      const mockPrisma: any = { activity: { findMany: jest.fn().mockResolvedValue([]), count: jest.fn().mockResolvedValue(0) } };
      (mockPrisma as any).working = mockPrisma;
      const r = new ActivityHeatmapReport(mockPrisma);
      const result = await r.generate(baseParams);
      const heatmap = result.charts.find((c: any) => c.type === 'HEATMAP');
      expect(heatmap).toBeDefined();
      const total = result.summary.find((m: any) => m.key === 'totalActivities');
      expect(total!.value).toBe(0);
    });
