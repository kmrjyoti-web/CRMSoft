...
(mockPrisma as any).working = mockPrisma;
    mockDrillDown = { getLeads: jest.fn() } as any;
    const resolver = { resolveLookupValues: jest.fn().mockImplementation((r: any) => Promise.resolve(r)), resolveUsers: jest.fn().mockImplementation((r: any) => Promise.resolve(r)) } as any;
    report = new RevenueReport(mockPrisma, mockDrillDown, resolver);
  });
