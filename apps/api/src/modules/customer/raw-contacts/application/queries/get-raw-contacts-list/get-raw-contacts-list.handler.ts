...
export class GetRawContactsListHandler implements IQueryHandler<GetRawContactsListQuery> {
    private readonly logger = new Logger(GetRawContactsListHandler.name);
  ...
  async execute(query: GetRawContactsListQuery) {
    try {
      ...
      this.logger.error(`GetRawContactsListHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
