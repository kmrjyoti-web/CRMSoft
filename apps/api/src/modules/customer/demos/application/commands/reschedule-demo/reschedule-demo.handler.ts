...
export class RescheduleDemoHandler implements ICommandHandler<RescheduleDemoCommand> {
    private readonly logger = new Logger(RescheduleDemoHandler.name);
  ...
  async execute(cmd: RescheduleDemoCommand) {
    try {
      ...
      this.logger.error(`RescheduleDemoHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
