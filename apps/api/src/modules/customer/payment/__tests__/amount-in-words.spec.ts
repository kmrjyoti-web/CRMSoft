...
describe('AmountInWordsService', () => {
  ...
  describe('edge cases', () => {
    it('should handle 1 paise (0.01)', () => {
      const result = service.convert(0.01);
      expect(result).toContain('Paise');
    });

    it('should handle exact 1 rupee', () => {
      expect(service.convert(1)).toContain('One');
      expect(service.convert(1)).toContain('Rupee');
    });

    it('should handle amount with only paise (0.50)', () => {
      const result = service.convert(0.50);
      expect(result).toContain('Fifty Paise Only');
    });
  });
});
