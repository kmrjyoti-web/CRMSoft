...
export class UpdateMenuCategoryHandler implements ICommandHandler<UpdateMenuCategoryCommand> {
    private readonly logger = new Logger(UpdateMenuCategoryHandler.name);
  ...
  async execute(command: UpdateMenuCategoryCommand) {
    try {
      ...
      this.logger.error(`UpdateMenuCategoryHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
