...
export class GetPortalAnalyticsHandler implements IQueryHandler<GetPortalAnalyticsQuery> {
    private readonly logger = new Logger(GetPortalAnalyticsHandler.name);
  ...
  async execute(query: GetPortalAnalyticsQuery) {
    try {
      ...
      this.logger.error(`GetPortalAnalyticsHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
