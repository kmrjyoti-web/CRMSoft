...
export class GetEligibleEntitiesHandler implements IQueryHandler<GetEligibleEntitiesQuery> {
    private readonly logger = new Logger(GetEligibleEntitiesHandler.name);
  ...
  async execute(query: GetEligibleEntitiesQuery) {
    try {
      ...
      this.logger.error(`GetEligibleEntitiesHandler failed: ${(error as Error).message}`, (error as Error).stack);
      throw error;
    }
  }
}
