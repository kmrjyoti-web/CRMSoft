...
describe('RbacEngine', () => {
  ...
  describe('error cases', () => {
    it('should propagate DB failure from rolePermission.findMany', async () => {
      prisma.rolePermission.findMany.mockRejectedValue(new Error('DB connection lost'));
      await expect(engine.check(ctx())).rejects.toThrow('DB connection lost');
    });

    it('should return false when role has empty permissions array', async () => {
      prisma.rolePermission.findMany.mockResolvedValue([]);
      const result = await engine.check(ctx({ action: 'leads:create' }));
      expect(result).toBe(false);
    });

    it('should return false for non-existent roleId with no permissions', async () => {
      prisma.rolePermission.findMany.mockResolvedValue([]);
      const result = await engine.check(ctx({ roleId: 'role-does-not-exist' }));
      expect(result).toBe(false);
    });
  });
});
