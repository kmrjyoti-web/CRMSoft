...
describe('ErrorCatalogService', () => {
  ...
  beforeEach(async () => {
    ...
    jest.clearAllMocks();
    mockErrorCatalog.findMany.mockResolvedValue(MOCK_ENTRIES);
  });
  ...
  describe('refreshCache', () => {
    it('should reload cache from DB', async () => {
      await service.onModuleInit();
      mockErrorCatalog.findMany.mockResolvedValue([MOCK_ENTRIES[0]]);
    ...
    it('should handle DB failure gracefully', async () => {
      await service.onModuleInit();
      mockErrorCatalog.findMany.mockRejectedValue(new Error('DB down'));
