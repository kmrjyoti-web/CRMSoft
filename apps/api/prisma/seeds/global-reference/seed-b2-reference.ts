/**
 * Seed script — Sprint B.2
 * Populates GlobalReferenceDB with:
 *   - 5 GST rate slabs
 *   - 50 common HSN/SAC codes
 *   - 11 currencies (INR default + top 10 global)
 *   - 18 timezones (Asia/Kolkata default + common)
 *   - 10 industry types
 *   - 15 languages (11 Indian + 4 global)
 *   - 20 sample Indian pincodes
 *
 * Run: npx ts-node --project tsconfig.json prisma/seeds/global-reference/seed-b2-reference.ts
 */

import { PrismaClient as GlobalReferenceClient } from '../../../node_modules/.prisma/global-reference-client';

const globalRef = new GlobalReferenceClient({
...
async function main() {
  ...
  for (const c of currencyData) {
    await globalRef.glCfgCurrency.upsert({
      ...
      create: {
        ...
        symbol: c.symbol,
        symbolAlt: (c as any).symbolAlt ?? null,
        decimalPlaces: (c as any).decimalPlaces ?? 2,
  ...
  for (const tz of timezoneData) {
    await globalRef.glCfgTimezone.upsert({
      ...
      create: {
        ...
        isDst: (tz as any).isDst ?? false,
        isDefault: (tz as any).isDefault ?? false,
  ...
  const languageData = [
    // Indian languages (Scheduled + widely spoken)
  ...
  for (const lang of languageData) {
    await globalRef.glCfgLanguage.upsert({
      ...
      create: {
        ...
        isDefault: (lang as any).isDefault ?? false,
  ...
  // ─── Sample PIN Codes ────────────────────────────────────────────────────────
  // Requires GlCfgState records (seeded in sprint B.1 seed-global.ts)
  // Only seed if India state records exist
