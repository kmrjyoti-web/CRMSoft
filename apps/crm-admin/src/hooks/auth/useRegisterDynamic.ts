...
export interface DynamicRegisterParams {
  combinedCode: string;
  brandCode: string;
  verticalCode: string;
  userType: string;
...
export function useRegisterDynamic() {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const registerDynamic = useCallback(async (params: DynamicRegisterParams): Promise<DynamicRegisterResult> => {
    setIsLoading(true);
    setError(null);
    try {
      const { data } = await api.post<ApiResponse<RegisterApiData>>(
        '/api/v1/auth/register-dynamic',
        params,
      );
  ...
  return { registerDynamic, isLoading, error };
}
